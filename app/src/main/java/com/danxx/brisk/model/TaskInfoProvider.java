	...
	public List<TaskInfo> GetAllTask(List<RunningAppProcessInfo> list) {
		...
		for (RunningAppProcessInfo appProcessInfo : list) {
			...
			try {
				...
				//不清除自己的进程
				if(!info.getPackageName().equalsIgnoreCase("com.danxx.brisktvlauncher") && !info.getPackageName().equalsIgnoreCase("com.yunos.tv.homeshell")){
